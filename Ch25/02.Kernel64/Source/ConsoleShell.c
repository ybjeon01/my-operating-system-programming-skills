...
static void kCreateFileInRootDirectory(const char *pcParameterBuffer) {
    ...
    if (
        (iLength > (sizeof(stEntry.vcFileName) - 1)) ||
        (iLength == 0)
    ) {
    ...
    /* allocate a cluster to new file */

    dwCluster = kFindFreeCluster();
    
    if (
        (dwCluster == FILESYSTEM_LASTCLUSTER) ||
